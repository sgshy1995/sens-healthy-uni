	...
	<view class="pain-q-a-wrapper">
		<view class="pain-q-a">
			<view class="pain-q-a-top">
				<u-search placeholder="搜索关键词" :showAction="false" v-model="keyword" @search="handleSearch"
					@clear="handleClearSearch"></u-search>
			</view>
			<view class="pain-q-a-top-tabs" :class="{sticky: sticky}">
				<u-tabs :list="listItems" lineWidth="30" lineColor="#4F68B0" :activeStyle="{
					            color: '#303133',
					            fontWeight: 'bold',
					            ...
					            color: '#606266',
					            transform: 'scale(1)'
					        }" @change="handleChangeTabs" :current="tabCurrent"
					itemStyle="padding-left: 15px; padding-right: 15px; height: 34px;">
				</u-tabs>
			</view>
			<view class="pain-q-a-in">
				<mescroll-uni ref="mescroll" @down="handleRefreshDown" :up="{onScroll:true, use: false}" @scroll="handleScroll" :fixed="false">
					<view class="pain-q-a-in-in">
						<view class="pain-q-a-top-empty"
							v-if="(!dataSource.length && tabCurrent === 0) || (!dataSourceCustom.length && tabCurrent === 1)">
							...
							<view class="pain-q-a-item" v-for="(u,index) in dataSource" :key="u.id"
								...
								<view class="pain-q-a-item-info" @click="handleShowDetail(u)">
									<image class="info-avatar"
										:src="u.anonymity ? avatarDefault : u.avatar ? baseUrl + u.avatar : avatarDefault"></image>
								...
								<view class="pain-q-a-item-content">
									...
									<view class="item-content-right" v-show="u.urls && u.urls.length">
										<!-- <u-album @click.stop.prevent :urls="u.urls" multipleSize="200rpx" singleSize="200rpx"
											rowCount="3" maxCount="3"></u-album> -->
										<s-album :urls="u.urls" :max="3"></s-album>
									</view>
								</view>
						...
						<view class="pain-q-a-item-wrapper" v-if="tabCurrent === 1 && dataSourceCustom.length">
							<view class="pain-q-a-item" v-for="(u,index) in dataSourceCustom" :key="u.id"
								:class="{'last-item': index === dataSource.length - 1}">
								<view class="pain-q-a-item-info" @click="handleShowDetail(u)">
									<image class="info-avatar"
										:src="u.anonymity ? avatarDefault : u.avatar ? baseUrl + u.avatar : avatarDefault"></image>
